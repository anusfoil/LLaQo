...
class RunnerIter(RunnerBase):
    ...
    def __init__(self, cfg, task, model, datasets, job_id):
        ...
        assert (self.iters_per_inner_epoch >
                0), "iters_per_inner_epoch must be greater than 0."
    ...
    def train(self):
        ...
        for start_iters in range(self.start_iters, self.max_iters,
                                 self.iters_per_inner_epoch):
            end_iters = start_iters + self.iters_per_inner_epoch

            # training phase
            if not self.evaluate_only:
                logging.info(
                    "Start training, max_iters={}, in total {} inner epochs.".
                    format(self.max_iters,
                           int(self.max_iters / self.iters_per_inner_epoch)))
            ...
            if len(self.valid_splits) > 0:
                for split_name in self.valid_splits:
                    ...
                    val_log = self.eval_epoch(
                        ...
                        cur_epoch=self._progress(end_iters))
    ...
    def _save_checkpoint(self, cur_iters, is_best=False):
        ...
        logging.info("Saving checkpoint at iters {} to {}.".format(
            cur_iters, save_to))
    ...
    def dataloaders(self) -> dict:
        ...
        if self._dataloaders is None:
            ...
            dataset_ratios = self.config.run_cfg.get("train_dataset_ratios",
                                                     None)
            ...
            if dataset_ratios is None:
                ...
                missing_keys = [
                    k for k in dataset_ratios if k not in self.datasets
                ]
                if len(missing_keys) > 0:
                    raise ValueError(
                        "Datasets with the following split names are not found: {}"
                        .format(missing_keys))

                unexpected_keys = [
                    k for k in self.datasets if k not in dataset_ratios
                ]
                if len(unexpected_keys) > 0:
                    raise ValueError(
                        "Datasets with the following split names are not expected: {}"
                        .format(unexpected_keys))

                dataset_ratios = [
                    float(dataset_ratios[k]) for k in self.datasets
                ]
                # NOTE: fix the code by replacing `self.dataset` with `dataset`
                datasets = reorg_datasets_by_split(self.datasets)
