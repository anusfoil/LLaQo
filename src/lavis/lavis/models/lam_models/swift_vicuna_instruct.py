...
_AVQA_PROMPT_ = "Choose the correct option to the following question: {question} Options: 0 {choice_0}; 1 {choice_1}; 2 {choice_2}; 3 {choice_3}. Answer:"


@registry.register_model("lam_vicuna_instruct")
class SWIFT_InstructBLIP(LAMBase, SWIFT):
    """
    LAM Vicuna model.
    ...
    # FIXME
    _VIT_CHECKPOINT_PATH_ = "ckpts/vit.pth"
    ...
    def __init__(
        ...
        self.llm_model.model.embed_tokens.weight[
            self.audio_indicator_id].zero_()
    ...
    def forward(self, samples):
        ...
        audio_indicator_tokens = self.audio_indicator_token.expand(
            inputs_llm.shape[0], -1, -1).to(audio.device)
        audio_indicator_atts = torch.ones(audio_indicator_tokens.size()[:-1],
                                          dtype=torch.long).to(audio.device)
    ...
    def generate(
        ...
        if audio.dim() == 5:
            inputs_llm, atts_llm = [], []
            for j in range(audio.size(2)):
                this_frame = audio[:, :, j, :, :]
                with self.maybe_autocast():
                    frame_embeds = self.ln_audio(
                        self.audio_encoder(this_frame)["patch_embedding"])
                ...
                frame_inputs_llm = self.llm_proj(
                    frame_query_output.last_hidden_state[:, :query_tokens.
                                                         size(1), :])
                frame_atts_llm = torch.ones(frame_inputs_llm.size()[:-1],
                                            dtype=torch.long).to(audio.device)
    ...
    def predict_answers(self,
        ...
        if prompt:
            if prompt.count("{}") == 2:
                if 'ocr_tokens' in samples:
                    text_input = [
                        prompt.format(', '.join(samples['ocr_tokens'][i][:30]),
                    ...
                    for i in range(len(samples["text_input"])):
                        ...
                        text_input.append(
                            prompt.format(samples["text_input"][i],
                                          this_choices))
            else:
                text_input = [
        ...
        output_text = self.generate(samples,
                                    num_beams=num_beams,
                                    max_length=max_len,
                                    min_length=min_len,
                                    length_penalty=length_penalty)

        if "apply_lemmatizer" in samples.keys(
    ...
    def _predict_class(
        ...
        query_tokens = self.query_tokens.expand(bs, -1, -1)
        if self.qformer_text_input:
            text_Qformer = self.tokenizer(prompt,
                                          padding='longest',
                                          truncation=True,
                                          max_length=self.max_txt_len,
                                          return_tensors="pt").to(audio.device)
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            for n in range(n_segments):
                ...
                this_llm_tokens, this_input_targets_len = self.concat_text_input_output(
                    this_input_tokens_ids, this_input_tokens_atts,
                    this_output_tokens_ids, this_output_tokens_atts)
                ...
                inputs_embeds = torch.cat([
                    inputs_llm.repeat_interleave(seg_len, dim=0), inputs_embeds
                ],
                                          dim=1)
                attention_mask = torch.cat([
                    atts_llm.repeat_interleave(seg_len, dim=0), this_llm_atts
                ],
                                           dim=1)

                this_targets = this_llm_input_ids.masked_fill(
                    this_llm_input_ids == self.llm_tokenizer.pad_token_id,
                    -100)
                ...
                this_targets = torch.cat([
                    empty_targets.repeat_interleave(seg_len, dim=0),
                    this_targets
                ],
                                         dim=1)
