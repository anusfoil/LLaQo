...
class LAMVicunaInstruct(LAMBase):
    ...
    def _single_forward(self, samples):
        """
        inputs_llm (query): get the aligned audio tokens and project into llm input space. 
        attn tokens
        text_input tokens: prompt after tokenizer
        """
        audio = samples["audio"] # (B, 1, 4096, 128)
    ...
    def generate(
        ...
        if audio.dim() == 4:
            ...
            with self.maybe_autocast():
                ...
                if next(self.llm_model.parameters()).dtype == torch.float16:
    ...
    def get_audio_embedding(self, audio, prompt=""):
        """
        self.query_tokens is the overall query. could include audio+text information
        audio: 
            - get the AudioMAE embedding and pass through layernorm 
        
        text prompt:
            - pass through tokenizer to get qformer tokens
        
        output is the return from qformer
        """
        B = audio.size(dim=0)
