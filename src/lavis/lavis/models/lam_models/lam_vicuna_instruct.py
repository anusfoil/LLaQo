...
class LAMVicunaInstruct(LAMBase):
    ...
    def forward(self, samples):
        def _single_wav_forward(sample):
            audio = sample["audio"]
            bs = audio.size(0)
            ...
            #     [
            #         t + self.llm_tokenizer.eos_token
            #         for t in sample['text_output']
            #     ],
            #     return_tensors="pt",
            #     padding="longest",
            #     truncation=True,
        ...
        # audio.shape = (B, n_audios_per_episode, C, T, F)
        audio = samples["audio"]
        if audio.dim() == 5:
            ...
            inputs_llm, atts_llm, text_input_tokens = _single_wav_forward(samples)
    ...
    #     # targets = torch.cat([empty_targets, targets], dim=1)
    #     # NOTE
    #     targets = torch.cat([targets[:, :1], empty_targets, targets[:, 1:]],
    #                         dim=1)
    ...
    #         [inputs_embeds[:, :1], inputs_llm, inputs_embeds[:, 1:]], dim=1)
    #     attention_mask = torch.cat([
    #         llm_tokens['attention_mask'][:, :1], atts_llm,
    #         llm_tokens['attention_mask'][:, 1:]
    #     ],
    #                                dim=1)
        ...
        for idx, (emb, batched_tokens) in enumerate(
            ...
            if idx == 0:
                inputs_embeds.append(
                    torch.cat(
                        [emb[:, :1, :], inputs_llm[:, idx, :, :], emb[:, 1:, :]], dim=1
                    )
                )
