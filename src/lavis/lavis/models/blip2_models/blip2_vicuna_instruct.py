...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def video_captioning(
        ...
        Args:
            samples (dict): A dictionary containing the following keys:
                - video (torch.Tensor): A tensor of shape (batch_size, n_frames, 3, H, W)
        ...
        Returns (dict):
                - embeddings (Tensor): A tensor of shape (batch_size, n_frames, 3, H, W)
                - captions (list): Strings of `batch_size` lists whose lengths are `num_captions`.
        """
        self.llm_tokenizer.padding_side = "left"
        ...
        assert (
            len(image.size()) == 5
        ), "INPUT should be frames, shape=(B, C, n_frames, W, H)"
        qformer_outputs = []
        inputs_llm, atts_llm = [], []
        for j in range(image.size(1)):
            ...
            frame_query_output_last_hidden_state = frame_query_output.last_hidden_state[
                :, : query_tokens.size(1), :
            ]
            qformer_outputs.append(
                frame_query_output_last_hidden_state.detach().clone().cpu()
            )
            frame_inputs_llm = self.llm_proj(frame_query_output_last_hidden_state)
            frame_atts_llm = torch.ones(
                frame_inputs_llm.size()[:-1], dtype=torch.long
            ).to(image.device)
            inputs_llm.append(frame_inputs_llm)
        ...
        qformer_outputs = torch.stack(
            qformer_outputs, dim=1
        )  # shape = (batch_size, n_frames, n_dim)
        inputs_llm = torch.cat(inputs_llm, dim=1)
        atts_llm = torch.cat(atts_llm, dim=1)
        ...
        return {
            "query_output": torch.tensor_split(qformer_outputs, B, dim=0),
            "inputs_llm": torch.tensor_split(inputs_llm, B, dim=0),
            "captions": [
    ...
    def av_generate(
        ...
        r"""Insert audio embeddings."""
        audio = samples["audio"]
        # bs = audio.size(0)
        # device = audio.device
        assert audio.device == device
        with self.maybe_autocast():
            audio_embeds = self.swift.ln_audio(
                self.swift.audio_encoder(audio)["patch_embedding"]
            )
        audio_atts = torch.ones(audio_embeds.size()[:-1], dtype=torch.long).to(device)

        swift_query_tokens = self.swift.query_tokens.expand(
            audio_embeds.shape[0], -1, -1
        )
        if self.qformer_text_input:
            text_swift = self.swift.tokenizer(
                prompt,
                padding="longest",
                truncation=True,
                max_length=self.max_txt_len,
            ...
            swift_query_atts = torch.ones(
                swift_query_tokens.size()[:-1], dtype=torch.long
            ).to(device)
            swift_atts = torch.cat([swift_query_atts, text_swift.attention_mask], dim=1)

            swift_output = self.swift.Qformer.bert(
                text_swift.input_ids,
                attention_mask=swift_atts,
                query_embeds=swift_query_tokens,
        ...
        inputs_swift_llm = self.swift.llm_proj(
            swift_output.last_hidden_state[:, : swift_query_tokens.size(1), :]
        )
        atts_swift_llm = torch.ones(inputs_swift_llm.size()[:-1], dtype=torch.long).to(
            device
        )
        # Add audio indicator
        audio_indicator_tokens = self.swift.audio_indicator_token.expand(
            inputs_swift_llm.shape[0], -1, -1
        ).to(device)
        audio_indicator_atts = torch.ones(
            audio_indicator_tokens.size()[:-1], dtype=torch.long
        ).to(device)
        inputs_swift_llm = torch.cat([audio_indicator_tokens, inputs_swift_llm], dim=1)
        atts_swift_llm = torch.cat([audio_indicator_atts, atts_swift_llm], dim=1)
        r"""Now back to LLM."""
        llm_tokens = self.llm_tokenizer(
            prompt, padding="longest", return_tensors="pt"
        ).to(device)
        with self.maybe_autocast():
            inputs_embeds = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
            inputs_embeds = torch.cat(
                [inputs_llm, inputs_swift_llm, inputs_embeds], dim=1
            )
            attention_mask = torch.cat(
                [atts_llm, atts_swift_llm, llm_tokens.attention_mask],
                dim=1,
            )
            # inputs_embeds = torch.cat([inputs_swift_llm, inputs_embeds], dim=1)
            # attention_mask = torch.cat(
            #     [atts_swift_llm, llm_tokens.attention_mask], dim=1)
    ...
    def av_generate_sep_prompt(
        ...
        r"""Insert audio embeddings."""
        audio = samples["audio"]
        audio_prompt = samples["audio_prompt"]
        if isinstance(audio_prompt, str):
            audio_prompt = [audio_prompt] * bs
        else:
            assert (
                len(audio_prompt) == bs
            ), "The number of audio prompts must be equal to the batch size."
        # bs = audio.size(0)
        # device = audio.device
        assert audio.device == device
        with self.maybe_autocast():
        ...
        r"""Now back to LLM."""
        llm_tokens = self.llm_tokenizer(
            audio_prompt, padding="longest", return_tensors="pt"
        ).to(device)
        with self.maybe_autocast():
            inputs_embeds = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
            inputs_embeds = torch.cat(
