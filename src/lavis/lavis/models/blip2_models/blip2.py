...
def compute_sim_matrix(model, data_loader, **kwargs):
    ...
    for i, sims in enumerate(
        ...
        image_inputs = vit_feats[start + i].repeat(k_test, 1,
                                                   1).to(model.device)
    ...
    if dist_utils.is_dist_avail_and_initialized():
        dist.barrier()
        torch.distributed.all_reduce(score_matrix_i2t,
                                     op=torch.distributed.ReduceOp.SUM)
        torch.distributed.all_reduce(score_matrix_t2i,
                                     op=torch.distributed.ReduceOp.SUM)
